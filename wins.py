#!/usr/bin/env python3
"""Sync wins from Apple Reminders to Obsidian daily notes."""
import subprocess
import sys
import time
import logging
...
LOG_FILE = Path("/tmp/wins-to-obsidian.log")
...
def get_reminders() -> list[tuple[str, datetime, str]]:
    ...
    log.info(f"Found {len(entries)} incomplete win(s)")
    return entries
...
def mark_reminders_complete(reminder_ids: list[str]) -> bool:
    ...
    log.info(f"Marking {len(reminder_ids)} win(s) as complete")
    ...
    if success:
        try:
            ...
            if marked_count != len(reminder_ids):
                ...
                log.info(f"Successfully marked {marked_count} win(s) complete")
        except ValueError:
...
def update_win_in_daily_note(entries: list[tuple[str, datetime, str]]) -> tuple[bool, list[str]]:
    """
    Update the One win from today section in the daily note.
    Returns (success, list of reminder IDs that were written).
    """
    if not entries:
        log.info("No wins to add")
        return True, []
    ...
    # Combine all wins (in case there are multiple)
    sorted_entries = sorted(entries, key=lambda x: x[1])
    wins_text = " ".join(note for _, _, note in sorted_entries)
    written_ids = [rid for rid, _, _ in sorted_entries]

    # Find the One win from today section
    section_marker = "**One win from today:**"
    ...
    # Find where the current win content ends (next line that doesn't start with >)
    ...
    new_content = (
        content[:marker_end] +
        "\n> " + wins_text + "\n" +
        ("\n" if after_win and not after_win.startswith("\n") else "") +
        after_win
    )
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            log.info(f"Wrote win to daily note (attempt {attempt})")

            # Verify write
            time.sleep(POST_WRITE_VERIFY_DELAY)
            verify_content = daily_note.read_text()

            if wins_text not in verify_content:
                log.warning("Verification failed - win not found")
...
def main() -> int:
    ...
    log.info("=" * 50)
    log.info("Starting wins-to-obsidian sync")
    ...
    if not entries:
        log.info("No wins to process")
        return 0

    log.info(f"Processing {len(entries)} win(s)")
    ...
    # Write to daily note
    success, written_ids = update_win_in_daily_note(entries)

    if not success:
        log.error("Failed to update daily note - wins NOT marked complete")
        return 1

    if not written_ids:
        log.info("No wins were written")
        return 0
    ...
    if mark_reminders_complete(written_ids):
        log.info(f"Successfully synced {len(written_ids)} win(s)")
        return 0
    else:
        log.error("Failed to mark wins complete - entries may duplicate on next run")
        return 1
