#!/usr/bin/env python3
"""Sync gratitude entries from Apple Reminders to Obsidian daily notes."""
import subprocess
import sys
import time
import logging
...
LOG_FILE = Path("/tmp/gratitude-to-obsidian.log")
...
def get_reminders() -> list[tuple[str, datetime, str]]:
    ...
    log.info(f"Found {len(entries)} incomplete gratitude item(s)")
    return entries
...
def mark_reminders_complete(reminder_ids: list[str]) -> bool:
    ...
    log.info(f"Marking {len(reminder_ids)} gratitude item(s) as complete")
    ...
    if success:
        try:
            ...
            if marked_count != len(reminder_ids):
                ...
                log.info(f"Successfully marked {marked_count} gratitude item(s) complete")
        except ValueError:
...
def update_gratitude_in_daily_note(entries: list[tuple[str, datetime, str]]) -> tuple[bool, list[str]]:
    """
    Update the 3 things I'm grateful for section in the daily note.
    Returns (success, list of reminder IDs that were written).
    """
    if not entries:
        log.info("No gratitude items to add")
        return True, []
    ...
    for i, (_, _, note) in enumerate(sorted_entries, 1):
        gratitude_lines.append(f"{i}. {note}")

    # Pad with empty entries if fewer than 3
    for i in range(len(sorted_entries) + 1, 4):
        gratitude_lines.append(f"{i}.")

    gratitude_text = "\n".join(gratitude_lines)

    # Find the 3 things I'm grateful for section
    section_marker = "**3 things I'm grateful for:**"
    ...
    # Find where the current gratitude content ends
    rest_of_content = content[marker_end:]
    lines = rest_of_content.split("\n")

    # Skip lines that are part of the gratitude section (numbered items or empty)
    end_idx = 0
    for i, line in enumerate(lines):
        stripped = line.strip()
        # Match numbered lines (1. ..., 2. ..., etc.) or empty lines
        if stripped == "" or (len(stripped) >= 2 and stripped[0].isdigit() and stripped[1] == "."):
            end_idx = i + 1
        elif stripped == "---":
    ...
    new_content = (
        content[:marker_end] +
        "\n" + gratitude_text + "\n" +
        ("\n" if after_gratitude and not after_gratitude.startswith("\n") and not after_gratitude.startswith("---") else "") +
        after_gratitude
    )
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            log.info(f"Wrote gratitude items to daily note (attempt {attempt})")

            # Verify write
            time.sleep(POST_WRITE_VERIFY_DELAY)
            verify_content = daily_note.read_text()

            # Check if first gratitude item is present
            first_item = sorted_entries[0][2] if sorted_entries else None
            if first_item and first_item not in verify_content:
                log.warning("Verification failed - gratitude items not found")
...
def main() -> int:
    ...
    log.info("=" * 50)
    log.info("Starting gratitude-to-obsidian sync")
    ...
    if not entries:
        log.info("No gratitude items to process")
        return 0

    log.info(f"Processing {len(entries)} gratitude item(s)")
    for rid, ts, note in entries:
        log.info(f"  - [{ts.strftime('%H:%M')}] {note[:60]}")

    if len(entries) > 3:
        log.warning(f"Found {len(entries)} gratitude items but only using first 3")

    # Small delay to let any pending Reminders sync complete
    time.sleep(1)

    # Write to daily note
    success, written_ids = update_gratitude_in_daily_note(entries)

    if not success:
        log.error("Failed to update daily note - gratitude items NOT marked complete")
        return 1

    if not written_ids:
        log.info("No gratitude items were written")
        return 0
    ...
    if mark_reminders_complete(written_ids):
        log.info(f"Successfully synced {len(written_ids)} gratitude item(s)")
        return 0
    else:
        log.error("Failed to mark gratitude items complete - entries may duplicate on next run")
        return 1
