#!/usr/bin/env python3
"""Sync intentions from Apple Reminders to Obsidian daily notes."""
import subprocess
import sys
import time
import logging
...
LOG_FILE = Path("/tmp/intention-to-obsidian.log")
...
def get_reminders() -> list[tuple[str, datetime, str]]:
    ...
    log.info(f"Found {len(entries)} incomplete intention(s)")
    return entries
...
def mark_reminders_complete(reminder_ids: list[str]) -> bool:
    ...
    log.info(f"Marking {len(reminder_ids)} intention(s) as complete")
    ...
    if success:
        try:
            ...
            if marked_count != len(reminder_ids):
                ...
                log.info(f"Successfully marked {marked_count} intention(s) complete")
        except ValueError:
...
def update_intention_in_daily_note(entries: list[tuple[str, datetime, str]]) -> tuple[bool, list[str]]:
    """
    Update the Today's Intention section in the daily note.
    Returns (success, list of reminder IDs that were written).
    """
    if not entries:
        log.info("No intentions to add")
        return True, []
    ...
    # Combine all intentions (in case there are multiple)
    sorted_entries = sorted(entries, key=lambda x: x[1])
    intentions_text = " ".join(note for _, _, note in sorted_entries)
    written_ids = [rid for rid, _, _ in sorted_entries]

    # Find the Today's Intention section
    section_marker = "**Today's Intention:**"
    ...
    # Find where the current intention content ends (next line that doesn't start with >)
    ...
    new_content = (
        content[:marker_end] +
        "\n> " + intentions_text + "\n" +
        ("\n" if after_intention and not after_intention.startswith("\n") else "") +
        after_intention
    )
    ...
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            log.info(f"Wrote intention to daily note (attempt {attempt})")

            # Verify write
            time.sleep(POST_WRITE_VERIFY_DELAY)
            verify_content = daily_note.read_text()

            if intentions_text not in verify_content:
                log.warning("Verification failed - intention not found")
...
def main() -> int:
    ...
    log.info("=" * 50)
    log.info("Starting intention-to-obsidian sync")
    ...
    if not entries:
        log.info("No intentions to process")
        return 0

    log.info(f"Processing {len(entries)} intention(s)")
    ...
    # Write to daily note
    success, written_ids = update_intention_in_daily_note(entries)

    if not success:
        log.error("Failed to update daily note - intentions NOT marked complete")
        return 1

    if not written_ids:
        log.info("No intentions were written")
        return 0
    ...
    if mark_reminders_complete(written_ids):
        log.info(f"Successfully synced {len(written_ids)} intention(s)")
        return 0
    else:
        log.error("Failed to mark intentions complete - entries may duplicate on next run")
        return 1
